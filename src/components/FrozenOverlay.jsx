...
export default function FrozenOverlay({ isVisible, expiryTime }) {
  const [timeLeft, setTimeLeft] = useState(0);

  useEffect(() => {
    if (!isVisible || !expiryTime) {
      setTimeLeft(0);
      return;
    }
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black/95 backdrop-blur-md pointer-events-auto"
        >
          ...
          <motion.div
            ...
            animate={{ scale: 1, y: 0 }}
            className="text-center p-8 border-2 border-red-600 bg-red-950/20 rounded-xl shadow-[0_0_50px_rgba(220,38,38,0.3)] max-w-md w-full mx-4"
          >
            <div className="mb-6">
              <svg 
            ...
            <h1 className="text-4xl font-black text-red-500 tracking-tighter mb-2 italic uppercase">
              SYSTEM FROZEN
            </h1>
            ...
            <div className="space-y-4">
              ...
              <div className="text-[10px] text-red-900 font-mono uppercase tracking-[0.3em] mt-4 leading-relaxed">
                Attempts to bypass will be logged <br/> Unauthorized access denied
              </div>
            </div>
          </motion.div>
          ...
          <div className="absolute bottom-10 left-0 right-0 overflow-hidden whitespace-nowrap opacity-10 pointer-events-none">
            <div className="animate-marquee inline-block text-red-600 font-mono text-xs">
              ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED ACCESS_DENIED
            </div>
          </div>
        </motion.div>
      )}
